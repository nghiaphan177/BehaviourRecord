...
{
    class AnalyzeAntecedentActivityServiceFakes : IAnalyzeAntecedentActivityService
    {
        private readonly List<AnalyzeAntecedentActivity> _activity;
        public AnalyzeAntecedentActivityServiceFakes()
        {
            _activity = new List<AnalyzeAntecedentActivity>()
            {
                new AnalyzeAntecedentActivity() { Id = new Guid("ab2bd817-98cd-4cf3-a80a-53ea0cd9c200"),
                    Content = "UnitTest1", CreateDate=DateTime.Now, UpdateDate=DateTime.Now},
                new AnalyzeAntecedentActivity() { Id = new Guid("815accac-fd5b-478a-a9d6-f171a2f6ae7f"),
                    Content = "UnitTest2", CreateDate=DateTime.Now, UpdateDate=DateTime.Now},
                new AnalyzeAntecedentActivity() { Id = new Guid("33704c4a-5b87-464c-bfb6-51971b4d18ad"),
                    Content = "UnitTest3", CreateDate=DateTime.Now, UpdateDate=DateTime.Now}
            };
        }
        public AnalyzeAntecedentActivityServiceFakes(List<AnalyzeAntecedentActivity> activity)
        {
            _activity = activity;
        }
        public async Task<ResponseResult<List<AnalyzeAntecedentActivity>>> Create(string content)
        {
            if (_activity.Any(prop => prop.Content == content))
                return new ResponseResultError<List<AnalyzeAntecedentActivity>>("Dữ liệu đã tồn tại");
            _activity.Add(new AnalyzeAntecedentActivity()
            {
                Id = Guid.NewGuid(),
                Content = content,
            ...
            return new ResponseResultSuccess<List<AnalyzeAntecedentActivity>>(_activity.ToList());
        }

        public async Task<ResponseResult<List<AnalyzeAntecedentActivity>>> Delete(string id)
        {
            if (!_activity.Any(prop => prop.Id.ToString() == id))
                return new ResponseResultError<List<AnalyzeAntecedentActivity>>("Id không tồn tại");
            var obj = _activity.Find(a => a.Id.ToString() == id);
            _activity.Remove(obj);
            return new ResponseResultSuccess<List<AnalyzeAntecedentActivity>>(_activity.ToList());
        }
