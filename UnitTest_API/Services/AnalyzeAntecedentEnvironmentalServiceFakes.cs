...
{
    class AnalyzeAntecedentEnvironmentalServiceFakes : IAnalyzeAntecedentEnvironmentalService
    {
        private readonly List<AnalyzeAntecedentEnvironmental> _environmental;
        public AnalyzeAntecedentEnvironmentalServiceFakes()
        {
            _environmental = new List<AnalyzeAntecedentEnvironmental>()
            {
                new AnalyzeAntecedentEnvironmental() { Id = new Guid("ab2bd817-98cd-4cf3-a80a-53ea0cd9c200"),
                    Content = "UnitTest1", CreateDate=DateTime.Now, UpdateDate=DateTime.Now},
                new AnalyzeAntecedentEnvironmental() { Id = new Guid("815accac-fd5b-478a-a9d6-f171a2f6ae7f"),
                    Content = "UnitTest2", CreateDate=DateTime.Now, UpdateDate=DateTime.Now},
                new AnalyzeAntecedentEnvironmental() { Id = new Guid("33704c4a-5b87-464c-bfb6-51971b4d18ad"),
                    Content = "UnitTest3", CreateDate=DateTime.Now, UpdateDate=DateTime.Now}
            };
        }
        public AnalyzeAntecedentEnvironmentalServiceFakes(List<AnalyzeAntecedentEnvironmental> environmental)
        {
            _environmental = environmental;
        }
        public async Task<ResponseResult<List<AnalyzeAntecedentEnvironmental>>> Create(string content)
        {
            if (_environmental.Any(prop => prop.Content == content))
                return new ResponseResultError<List<AnalyzeAntecedentEnvironmental>>("Dữ liệu đã tồn tại");
            _environmental.Add(new AnalyzeAntecedentEnvironmental()
            {
                Id = Guid.NewGuid(),
                Content = content,
